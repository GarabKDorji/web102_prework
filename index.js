...
const numberOfUnfunded = unfunded.length;
// create a string that explains the number of unfunded games using the ternary operator
const summary = `
  A total of ${totalPledged} has been raised for ${totalGames} game${numberOfUnfunded !== 1 ? 's' : ''}.
  Currently, ${numberOfUnfunded} game${numberOfUnfunded !== 1 ? 's remain' : ' remains'} unfunded.
`;
...
searchForm.addEventListener('submit', function(event) {
    ...
    const game = GAMES_JSON.find((element) => { 
        return searchInput.value.toLowerCase() == element.name.toLowerCase() 
    })
    console.log([game])
    addGamesToPage([game]);
});